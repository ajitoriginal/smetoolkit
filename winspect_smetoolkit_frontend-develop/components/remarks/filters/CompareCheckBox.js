...
const CompareCheckBox = () => {
  ...
  const setShowCompareStore = useShowCompareStore(
    (state) => state.setShowCompare,
  );
  const [checked, setChecked] = useState(isCustomStore);
  useEffect(() => {
    setShowCompareStore(checked);
  }, [checked]);
  ...
  return (
    <label className={styles.compareCheckbox}>
      ...
      <span className={styles.text}>Show Comparison</span>
    </label>
  );
};
