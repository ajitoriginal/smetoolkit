import { useEffect, useState } from "react";
import useRemarkStore from "../../../stores/remarkStore";
import styles from "./multipleRemarkActionPopup.module.scss";
const MultipleRemarkActionPopup = () => {
  const [showMoveTemplateBtn, setShowMoveTemplateBtn] = useState(true);

  const {
    selectedCheckboxesRemark,
    setShowMoveRemarkPopup,
    setShowDeleteRemarkPopup,
    setSelectedRemarks,
  } = useRemarkStore();

  useEffect(() => {
    let showMoveTemplateBtn = true;
    selectedCheckboxesRemark.map((remark) => {
      if (remark.remarkType == "Template") {
        showMoveTemplateBtn = false;
      }
    });
    setShowMoveTemplateBtn(showMoveTemplateBtn);
  });

  const handleDeleteClick = () => {
    setShowDeleteRemarkPopup(true);
    setSelectedRemarks(selectedCheckboxesRemark);
  };

  const handleMoveToTemplateClick = () => {
    setShowMoveRemarkPopup(true);
    setSelectedRemarks(selectedCheckboxesRemark);
  };
  if (selectedCheckboxesRemark.length > 0) {
    return (
      <div className={styles.popupWrapper}>
        <button onClick={handleDeleteClick} className={styles.red}>
          <img src="/images/delete2.svg" />
          Delete Remarks
        </button>
        {showMoveTemplateBtn ? (
          <button onClick={handleMoveToTemplateClick}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path
                d="M13.5 9.75L12.4425 10.8075L14.3775 12.75H7.5V14.25H14.3775L12.4425 16.1925L13.5 17.25L17.25 13.5L13.5 9.75Z"
                fill="white"
              />
              <path
                d="M8.379 4.5L10.9395 7.0605L11.379 7.5H21V19.5H3V4.5H8.379ZM8.379 3H3C2.60218 3 2.22064 3.15804 1.93934 3.43934C1.65804 3.72064 1.5 4.10218 1.5 4.5V19.5C1.5 19.8978 1.65804 20.2794 1.93934 20.5607C2.22064 20.842 2.60218 21 3 21H21C21.3978 21 21.7794 20.842 22.0607 20.5607C22.342 20.2794 22.5 19.8978 22.5 19.5V7.5C22.5 7.10218 22.342 6.72064 22.0607 6.43934C21.7794 6.15804 21.3978 6 21 6H12L9.4395 3.4395C9.15826 3.15818 8.77679 3.00008 8.379 3Z"
                fill="white"
              />
            </svg>
            Move to Template
          </button>
        ) : (
          <button disabled>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path
                d="M13.5 9.75L12.4425 10.8075L14.3775 12.75H7.5V14.25H14.3775L12.4425 16.1925L13.5 17.25L17.25 13.5L13.5 9.75Z"
                fill="white"
              />
              <path
                d="M8.379 4.5L10.9395 7.0605L11.379 7.5H21V19.5H3V4.5H8.379ZM8.379 3H3C2.60218 3 2.22064 3.15804 1.93934 3.43934C1.65804 3.72064 1.5 4.10218 1.5 4.5V19.5C1.5 19.8978 1.65804 20.2794 1.93934 20.5607C2.22064 20.842 2.60218 21 3 21H21C21.3978 21 21.7794 20.842 22.0607 20.5607C22.342 20.2794 22.5 19.8978 22.5 19.5V7.5C22.5 7.10218 22.342 6.72064 22.0607 6.43934C21.7794 6.15804 21.3978 6 21 6H12L9.4395 3.4395C9.15826 3.15818 8.77679 3.00008 8.379 3Z"
                fill="white"
              />
            </svg>
            Move to Template
          </button>
        )}
      </div>
    );
  }
};

export default MultipleRemarkActionPopup;
