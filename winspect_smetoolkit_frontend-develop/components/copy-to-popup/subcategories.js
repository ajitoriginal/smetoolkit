...
import AboutValueServices from "../../api/services/AboutValueServices";
...
const SubCategories = () => {
  ...
  const [filterSubCategories, setSubFilteredCategories] = useState([]);
  ...
  const getSubCategories = async () => {
    ...
    if (result.body.length > 0) {
      setSubFilteredCategories(result.body);
    } else {
      setSubFilteredCategories(null);
    }
  };
  ...
  const delayedCategorySearch = debounce((e) => {
    ...
    if (searchText !== "") {
      setSubFilteredCategories([...filtered]);
    } else {
      setSubFilteredCategories([...subCategories]);
    }
  }, 300);
  ...
  const handleCopyClick = async () => {
    ...
    let data = {
      ...
      copyFromAboutIds: selectedAboutToCopy,
      copyToDraftSubCategoryId: selectedSubCategory.templateSubCategoryId,
    };

    if (copyTo == "remark") {
      data = {
        copyFromPubTemplateId: template,
        copyFromRemarkIds: selectedRemarkToCopy,
        copyToDraftSubCategoryId: selectedSubCategory.templateSubCategoryId,
      };
      result = await RemarkServices.copyRemark(data, router, true);
    } else {
      result = await AboutValueServices.copyAboutValues(data, router, true);
    }
    setLoader(false);
    close();
  ...
  let count = selectedAboutToCopy.length;
  if (copyTo == "remark") {
  ...
  return (
    <div>
      ...
      <div className={copyStyle.categoryListWrapper}>
        <div className={copyStyle.backButton}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
          ...
          <svg
            ...
            viewBox="0 0 7 12"
            fill="none"
            className={copyStyle.breadCrumbArrowIcon}
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
        ...
        <div
          className={`${copyStyle.categoryList} ${copyStyle.subCategoryList}`}
        >
          {filterSubCategories !== null ? (
            <>
              <div className={copyStyle.selectTitle}>Select Sub-Category</div>
                ...
                return (
                  <div
                    ...
                    className={`${copyStyle.categoryItem} ${
                      selectedSubCategory?.templateSubCategoryId ===
                        item.templateSubCategoryId && copyStyle.activeCategory
                    }`}
                  >
                    <div className={copyStyle.leftPart}>
                      <div className={copyStyle.categoryName}>{item.name}</div>
                    </div>
